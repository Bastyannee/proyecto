...
void dibujar_rect(int x, int y, int w, int h, int r, int g, int b) {
    ...
    SDL_SetRenderDrawColor(renderizador, 0, 0, 0, 255); // Borde negro opcional
...
void dibujar_texto(const char *texto, int x, int y, int r, int g, int b) {
    ...
    if (surface) {
        ...
        SDL_RenderCopy(renderizador, texture, NULL, &dst);
...
void dibujar_muro_destruible(int x, int y) {
    // Color ladrillo (Rojo/Naranja)
    dibujar_rect(x, y, TILE_SIZE, TILE_SIZE, 178, 34, 34);
    // Lineas de ladrillo simuladas
    SDL_SetRenderDrawColor(renderizador, 0, 0, 0, 255);
    SDL_RenderDrawLine(renderizador, x, y + TILE_SIZE/2, x + TILE_SIZE, y + TILE_SIZE/2);
    SDL_RenderDrawLine(renderizador, x + TILE_SIZE/2, y, x + TILE_SIZE/2, y + TILE_SIZE);
}

void dibujar_muro_solido(int x, int y) {
    // Color acero/gris
    dibujar_rect(x, y, TILE_SIZE, TILE_SIZE, 169, 169, 169);
    // Un cuadro interior para dar efecto de bloque duro
    SDL_Rect interior = {x + 5, y + 5, TILE_SIZE - 10, TILE_SIZE - 10};
    SDL_SetRenderDrawColor(renderizador, 100, 100, 100, 255);
    SDL_RenderFillRect(renderizador, &interior);
}
